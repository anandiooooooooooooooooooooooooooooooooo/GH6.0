...
// Komponen UI untuk Dashboard (didefinisikan di file yang sama)
function Dashboard({ userFullName }: { userFullName: string }) {
  const cardVariants = {
  ...
  return (
    <div className="min-h-screen w-full bg-[#CCC1B8]/30">
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const checkUserStatus = async () => {
      ...
      const onboardingStatus = profile?.has_completed_onboarding || false;
      setHasCompletedOnboarding(onboardingStatus);
      setUserFullName(profile?.full_name || "Pengguna Baru");
  ...
  const handleOnboardingComplete = () => {
    ...
    setHasCompletedOnboarding(true); // Langsung update UI tanpa perlu refresh
  };

  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#CCC1B8]/30">
